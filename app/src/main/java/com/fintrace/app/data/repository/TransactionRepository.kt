...
class TransactionRepository @Inject constructor(
    ...
    /**
     * Private helper to calculate breakdown from a list of transactions.
     * Centralizes the income/expense calculation logic.
     */
    private fun calculateBreakdown(transactions: List<TransactionEntity>): MonthlyBreakdown {
    ...
    /**
     * Private helper to get breakdown for a date range.
     * Used by getCurrentMonthBreakdown and getLastMonthBreakdown.
     */
    private fun getBreakdownForPeriod(
        startDate: LocalDateTime,
        endDate: LocalDateTime
    ): Flow<MonthlyBreakdown> {
        return transactionDao.getTransactionsBetweenDates(startDate, endDate)
            .map { transactions -> calculateBreakdown(transactions) }
    }

    /**
     * Private helper to get currency-grouped breakdown for a date range.
     * Used by getCurrentMonthBreakdownByCurrency and getLastMonthBreakdownByCurrency.
     */
    private fun getBreakdownByCurrencyForPeriod(
        startDate: LocalDateTime,
        endDate: LocalDateTime
    ): Flow<Map<String, MonthlyBreakdown>> {
        return transactionDao.getTransactionsBetweenDates(startDate, endDate)
            .map { transactions ->
                transactions.groupBy { it.currency }
                    .mapValues { (_, currencyTransactions) -> calculateBreakdown(currencyTransactions) }
            }
    }
    ...
    fun getCurrentMonthBreakdown(): Flow<MonthlyBreakdown> {
        ...
        return getBreakdownForPeriod(startDate, endDate)
    }
    ...
    fun getCurrentMonthBreakdownByCurrency(): Flow<Map<String, MonthlyBreakdown>> {
        ...
        val startDate = now.withDayOfMonth(1).atStartOfDay()
        val endDate = now.atTime(23, 59, 59)
        return getBreakdownByCurrencyForPeriod(startDate, endDate)
    }
