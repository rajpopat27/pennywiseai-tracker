package com.fintrace.app.data.backup
...
import com.fintrace.app.data.database.entity.*
...
class BackupImporter @Inject constructor(
    ...
    private val database: FintraceDatabase,
     ...
     */
    private suspend fun readBackupFile(uri: Uri): FintraceBackup {
        return withContext(Dispatchers.IO) {
            context.contentResolver.openInputStream(uri)?.use { inputStream ->
                val reader = BufferedReader(InputStreamReader(inputStream))
                val content = reader.readText()
                gson.fromJson(content, FintraceBackup::class.java)
     ...
     */
    private fun isCompatibleVersion(backup: FintraceBackup): Boolean {
        // For now, accept all v1.x backups
        return backup.format.startsWith("Fintrace Backup v1")
    }
    
    /**
     * Replace all existing data with backup data
     */
    private suspend fun replaceAllData(backup: FintraceBackup): ImportResult {
     ...
     */
    private suspend fun mergeData(backup: FintraceBackup): ImportResult {
        var importedTransactions = 0
        var importedCategories = 0
