package com.fintrace.app.worker
...
import com.fintrace.parser.core.bank.HDFCBankParser
import com.fintrace.parser.core.bank.IndianBankParser
import com.fintrace.parser.core.bank.SBIBankParser
import com.fintrace.parser.core.bank.IndusIndBankParser
...
class OptimizedSmsReaderWorker @AssistedInject constructor(
    ...
    private val pendingTransactionManager: com.fintrace.app.data.manager.PendingTransactionManager
...
private suspend fun processSubscriptionNotifications(
    parser: com.fintrace.parser.core.bank.BankParser,
    sms: SmsMessage,
...
private suspend fun saveParsedTransaction(
    ...
    return try {
        ...
        if (confirmationEnabled && !bypassForScans) {
            ...
            return when (result) {
                is com.fintrace.app.data.manager.PendingTransactionManager.AddPendingResult.SavedAsPending -> {
                    Log.d(TAG, "Transaction saved to pending queue: ${result.id}")
                    true
                }
                is com.fintrace.app.data.manager.PendingTransactionManager.AddPendingResult.ShowDialog -> {
                    Log.d(TAG, "Transaction saved to pending queue (foreground)")
                    true
                }
                is com.fintrace.app.data.manager.PendingTransactionManager.AddPendingResult.Duplicate -> {
                    Log.d(TAG, "Transaction is duplicate: ${result.reason}")
                    false
                }
                is com.fintrace.app.data.manager.PendingTransactionManager.AddPendingResult.DirectSaved -> {
                    Log.d(TAG, "Transaction saved directly: ${result.transactionId}")
                    true
                }
                is com.fintrace.app.data.manager.PendingTransactionManager.AddPendingResult.Error -> {
...
private suspend fun processBalanceUpdate(
    parsedTransaction: ParsedTransaction,
    entity: com.fintrace.app.data.database.entity.TransactionEntity,
    rowId: Long
) {
    if (parsedTransaction.accountLast4 != null) {
        ...
        val targetAccountLast4 = if (isFromCard) {
            ...
            val result = when {
                card.cardType == com.fintrace.app.data.database.entity.CardType.CREDIT -> {
                    // Credit cards get balance entries
                    Log.d(TAG, "CREDIT card - will create balance entry")
                    parsedTransaction.accountLast4
                }

                card.cardType == com.fintrace.app.data.database.entity.CardType.DEBIT && card.accountLast4 != null -> {
        ...
        if (targetAccountLast4 != null) {
            val isCreditCard = (parsedTransaction.type.toEntityType() == TransactionType.EXPENSE) ||
                    ...
                    } == com.fintrace.app.data.database.entity.CardType.CREDIT
