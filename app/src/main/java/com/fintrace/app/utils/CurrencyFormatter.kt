...
object CurrencyFormatter {
    ...
    fun getBankBaseCurrency(bankName: String?): String {
        if (bankName == null) return Currency.DEFAULT

        // Try to find a parser that can handle this bank name
        val parser = BankParserFactory.getParserByName(bankName)
        return parser?.getCurrency() ?: Currency.DEFAULT
    }

    /**
     * Sorts a list of currency codes with the priority currency first, then alphabetically.
     * This is the standard sorting for currency lists throughout the app.
     *
     * @param currencies List of currency codes to sort
     * @param priorityCurrency The currency to prioritize first (default: Currency.DEFAULT which is INR)
     * @return Sorted list with priority currency first (if present), then alphabetically
     *
     * Example:
     * ```
    ...
    fun sortCurrencies(
        ...
        return currencies.sortedWith { a, b ->
            when {
                a == priorityCurrency -> -1 // Priority currency first
